...
import { Download, Link as LinkIcon, Loader2, Image as ImageIcon, Film } from 'lucide-react';
...
const PinterestDownloader = () => {
  ...
  const fetchPinterestData = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch Pinterest content');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className='pt-10'>
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          <button
            ...
            className="flex-1 md:flex-none inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
            {isLoading ? (
              <>
          ...
          {pinterestUrl && (
            <button
              onClick={resetForm}
              disabled={isLoading}
              className="flex-1 md:flex-none inline-flex justify-center items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md shadow-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Clear
            </button>
          )}
                ...
                >
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex justify-between items-center">
                      ...
                      <a
                        href={item.url}
                        download={getFileNameFromUrl(item.url, item.type)}
        ...
        {mediaItems.length === 0 && !isLoading && !error && (
